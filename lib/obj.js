    ...
    req.sharks.forEach(function (s) {
        ...
        s.once('response', function onSharkResult(sres) {
            ...
            if (sres.statusCode === 469) {
                ...
                next_err(new restifyErrors.BadRequestError(
                    'Content-MD5 invalid'));
...
function saveMetadata(req, res, next) {
    ...
    common.createMetadata(req, 'object', function (err, opts) {
        ...
        opts.previousMetadata = req.metadata;
            /*
             * Let the delete mechanism know whether there might be
             * SnapLinks that still exist for this account, so it can
             * treat the object differently.
             */
            var uuid = req.owner.account.uuid;
            common.checkAccountSnaplinksMightExist(req, uuid,
                function (mightExist) {
                ...
                if (!mightExist) {
                    ...
                    // Note: `Disabled` is historical here. This can all
                    // go away once cleanup has been run everywhere and
                    // we don't need to worry about SnapLinks any more.
        ...
        req.moray.putMetadata(opts, function (err2, obj, data) {
            ...
            if (err2) {
                ...
                /*
                 * The following headers will have already been set to the
                 * previous metadata values if they existed (for example, if
                 * the object already existed and is being overwritten) -
                 * specifically Etag and Last-Modified are set earlier by the
                 * getMetadata handler in common.js.
                 *
                 * Because of this, they should be removed before being set to
                 * the new values to ensure multiple headers are not sent for
                 * the following keys.
                 *
                 * restify/node-restify#693 makes restify's
                 * response.header(name, value) append new header values as
                 * opposed to overwriting them.
                 */
                res.removeHeader('Etag');
                res.removeHeader('Last-Modified');
                res.removeHeader('Computed-MD5');
...
function negotiateContent(req, res, next) {
    ...
    /* BEGIN JSSTYLED */
    // Workaround a combination of (a) questionable (arguably broken)
    // `contentType` values on some Manta objects seen in the wild and (b)
    // a behaviour change in the "negotiator" module used by restify, such
    // that a request with `*/*` in the Accept header can *fail* to accept
    // the object's content-type.
    //
    // As part of regular content negotiation, a request for a Manta object will
    // match a given `Accept: ...` header value to the object's content type
    // (`contentType` in the database).
    //
    // Starting about restify 4.x, its content negotiation handling upgraded
    // to negotiator@^0.6.1. That version included this commit:
    //      https://github.com/jshttp/negotiator/commit/1cd03e552629a933f44
    // that slightly changed how `contentType` is parsed. The negotiator module
    // is written such that if the contentType cannot be parsed, then any
    // `Accept` header value fails. This happens even if the header value
    // includes `*/*` -- meant to match any content type.
    //
    // In SPC it was observed that a number of objects (presumably accidentally)
    // are set to "/image/jpeg", instead of the correct "image/jpeg". We are
    // not positive that is the only erroneous content-type. Current Manta Java
    // and Node.js SDKs set `Accept` by default. That means that an object
    // becomes unreachable if an unparsable contentType is set.
    //
    // WORKAROUND: If `contentType` is unparseable, default to
    // `application/octet-stream`. This is the same default content-type applied
    // to uploaded objects if no content-type is given at the time.
    //
    // Dev Note: The "negotiator" module doesn't export its parse function,
    // so we'll use the separate "content-type" module and *hope* they agree.
    // Both modules are currently maintained by the same person, FWIW.
    /* END JSSTYLED */
    var contentType = req.metadata.contentType;
    assert.string(contentType, 'req.metadata.contentType');
    try {
        parseContentType(contentType);
    } catch (parseErr) {
        req.log.debug({objectPath: req.url, contentType: contentType},
            'workaround invalid object contentType');
        contentType = 'application/octet-stream';
    }

    if (!req.accepts(contentType)) {
        return (next(new NotAcceptableError(req, contentType)));
    }
        ...
        if (!connected) {
            ...
            if (savedErr && savedErr._result) {
                ...
                if (req.headers['range'] !== undefined && rh['content-range']) {
                    ...
                    next(new restifyErrors.RangeNotSatisfiableError());
                    return;
                }
            }
...
function deletePointer(req, res, next) {
    ...
    var counter =
        req.collector.getCollector(common.METRIC_DELETED_OBJECT_COUNTER);
    var log = req.log;
    var opts = {
        key: req.key,
    ...
    assert.object(counter, 'METRIC_DELETED_OBJECT_COUNTER');
    ...
    req.moray.delMetadata(opts, function (err) {
        if (err) {
            ...
            // Bump the object deletion counter since we succeeded in deleting.
            counter.increment();
